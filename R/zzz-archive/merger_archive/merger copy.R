...
# ahha %>%
#   filter(time_point == "t2" & PID %>% str_detect("A|B|C")) %>%
#   get_dupes(time_point, PID) %>%
#   select(time_point, PID, starts_with("date")) %>%
#   View
...
source(here("R", "functions_merger.R"))
## NB need to lowercase FID to fid and change everywhere for this to work
df <- df %>% add_parent_suffixed(vars_to_suffix)
